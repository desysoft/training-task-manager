...
public class TaskDao extends AbstractDao implements PanacheRepositoryBase<Task, String> {
    ...
    public List<Task> findAllTask() {
        try {
            List<Task> lstTasks = find("status = ?1", ParametersConfig.status_enable).list();
            lstTasks.stream().forEach(task -> task.setTaskCompletionRate());
            return lstTasks;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public List<Task> findAllTask(String search_value) {
        try {
            List<Task> lstTasks = find("(name LIKE :search_value OR description LIKE :search_value OR code LIKE :search_value) AND status = :status", Parameters.with("search_value", "%" + search_value + "%").and("status", ParametersConfig.status_enable)).list();
            lstTasks.stream().forEach(task -> task.setTaskCompletionRate());
            return lstTasks;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public List<Activity> findAllActivitiesByTask(String id_task){
        try {
            return OActivityDao.findAllByTask(id_task);
        }catch (Exception e){
            e.printStackTrace();
        }
        return null;
    ...
    public List<Task> getPersonTasks(Person person) {
        try {
            List<Task> lstTasks =  this.getEm().createQuery("SELECT t FROM Task t WHERE t.OProjectPerson.OPerson.id = ?1 AND t.status <> ?2 ")
    ...
    public Task addListActivityInTask(Task task, List<Activity> lstActivities) {
        try {
            ...
            lstActivities.stream().forEach(activity -> {
                Activity oActivity = OActivityDao.findByIdCustom(activity.id).orElse(null);
                if (oActivity != null) OActivityDao.addActivityInTask(oTask, oActivity);
            });
            this.setMessage(ParametersConfig.PROCESS_SUCCES);
            this.setDetailMessage(ParametersConfig.SUCCES_UPDATE);
            return oTask;
